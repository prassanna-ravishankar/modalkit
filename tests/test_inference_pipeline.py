...
class TextProcessingPipeline(InferencePipeline):
    ...
    def preprocess(self, input_list: list[BaseModel]) -> dict:
        ...
        for input_item in input_list:
            ...
            # Cast to SentimentInput since we know that's what we're working with
            clean_text = input_item.text.strip().lower()  # type: ignore  # text attribute exists on SentimentInput
    ...
    def test_pipeline_method_signatures_are_enforced(self):
        ...
        class SignatureTestPipeline(InferencePipeline):
            def __init__(self):
                ...
                super().__init__("test", tempfile.gettempdir(), {})
                self.preprocess_args: tuple = ()
                self.predict_args: tuple = ()
                self.postprocess_args: tuple = ()
